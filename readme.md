...
function create() {
    ...
    // перемещаем спрайт "на поверхность", чтоб он был поверх всего прочего
    player.bringToFront();

    // перемещаем спрайт "вглубь", чтоб он был позади всего прочего
    player.sendToBack();
...
function create() {
    ...
    // теперь координаты задают не верхний левый угол текста, а центр
    text.makeXYCentred();

    // перемещаем текст "на поверхность", чтоб он был поверх всего прочего
    text.bringToFront();

    // перемещаем текст "вглубь", чтоб он был позади всего прочего
    text.sendToBack();
}
```
